...
import petfactory.modelling.pipe_tool.build_pipes as build_pipes
reload(build_pipes)
...
class Curve_spreadsheet(QtGui.QWidget):
    ...
    def add_fitting_mesh_click(self, lineedit):
        ...
        if not pet_verify.verify_selection(pm.nodetypes.Mesh):
            pm.warning('Please select a Mesh!')
            return
               
        node = pet_verify.to_transform(sel_list[0])
        lineedit.setText(node.longName())
    
    def add_path_click(self, lineedit):
        ...
        crv = pet_verify.to_transform(sel_list[0])
 
        crv_name = crv.longName()    
        lineedit.setText(crv_name)
    ...
    def on_build_click(self):
        ...
        build_pipes.add_radius_list_to_crv(crv, corner_radius_list)
        ...
        # build mesh 
        mesh_mobj_list = build_pipes.build_mesh(pos_list)
